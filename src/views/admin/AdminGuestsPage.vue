<template>
  <section class="space-y-6 animate-fade-in">
    <div v-if="stats" class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <article class="admin-stat-card admin-stat-card--emerald">
        <p class="admin-kpi-label">{{ t('admin.dashboard.yesConfirmations') }}</p>
        <p class="admin-kpi-value">{{ stats.confirmations_oui }}</p>
      ...
      <article class="admin-stat-card admin-stat-card--rose">
        ...
        <p class="admin-kpi-value">{{ stats.confirmations_non }}</p>
      </article>
      <article class="admin-stat-card admin-stat-card--amber">
        <p class="admin-kpi-label">{{ t('admin.dashboard.pending') }}</p>
        <p class="admin-kpi-value">{{ stats.en_attente }}</p>
      </article>
      <article class="admin-stat-card admin-stat-card--violet">
        <p class="admin-kpi-label">{{ t('admin.dashboard.totalGuests') }}</p>
        <p class="admin-kpi-value">{{ stats.total_invites }}</p>
      </article>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-[1.3fr_0.7fr]">
      ...
      <div class="admin-panel">
        ...
        <div class="mt-4 grid gap-2">
          <button class="admin-btn-soft justify-start" @click="applyPreset('pending')">En attente</button>
          <button class="admin-btn-soft justify-start" @click="applyPreset('confirmed')">Confirmés</button>
          <button class="admin-btn-soft justify-start" @click="applyPreset('declined')">Refus</button>
          <button class="admin-btn-soft justify-start" @click="applyPreset('reset')">Réinitialiser les filtres</button>
        </div>
      </div>
    </div>
    ...
    <div class="admin-panel overflow-x-auto">
      ...
      <table v-else class="w-full min-w-[780px] text-sm">
        <thead class="text-slate-600 uppercase text-xs tracking-wide">
          <tr>
